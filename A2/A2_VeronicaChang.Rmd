...
After our previous analysis in A1, the final dataset was made up of:

    - 15076 with unique identifiers
...
In the previous assignment, A1, we used the dataset ID [GSE111972](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE111972), from the publication "Transcriptional profiling of human microglia reveals grey-white matter heterogeneity and multiple sclerosis-associated changes" by van der Poel et.
al (@vanderPoel2019).
In A1, we conducted data cleaning, normalization, gene mapping, and preliminary analysis.
We filtered out the lowly expressed genes and were left with 15076 genes compared to the original 21283.
A total of 6207 genes were removed.
The number of genes did not change after normalization.
Genes were already mapped to HUGO gene symbols.
When we generated a MDS plot post-normalization, we saw that the tissues GM and WM clustered relatively well with each other regardless of which patient group they were obtained from.

```{r message = FALSE, tidy=TRUE, warning=FALSE}
knitr::include_graphics("~/projects/A2/figures/a1_mdsplot.png")
```
...
We already defined a group that has both of these factors in A1.
...
We will be testing for differential expression between the MS and CON samples for each tissue.
We want to also adjust the p-values using the Benjamini-Hochberg method as we are doing multiple comparisons.
This method assumes that the power to detect differential expression is equally likely among all the tests - reducing the false discovery rate.
...
Interestingly, there are not many DE genes for the GM comparison - only 4 and all of them are up-regulated.
For the WM comparison, there are none.
This makes sense as in the MDS we obtained from A1, we saw that the samples cluster together by tissue despite the patient group.
From now on, our analysis will be focused on comparing the GM and WM in the two patient groups.

## GM vs WM

We will be testing for differential expression between the different tissues for each patient group.
We want to also adjust the p-values using the Benjamini-Hochberg method as we are doing multiple comparisons.
This method assumes that the power to detect differential expression is equally likely among all the tests - reducing the false discovery rate.
...
# Plot volcano plot
ggplot2::ggplot(results_ms, aes(logFC, -log(FDR, 10))) + labs(title = "A. Volcano plot of differentially expressed genes in grey matter and 
white matter tissues in MS patients") +
...
# Plot volcano plot
ggplot2::ggplot(results_con, aes(logFC, -log(FDR, 10))) + labs(title = "B. Volcano plot of differentially expressed genes in grey matter and 
white matter tissues in CON patients") +
    ggplot2::geom_point(aes(color = Expression), size = 2/5) +
    xlab(expression("log"[2]*"FC")) + 
    ylab(expression("-log"[10]*"FDR")) + 
    ggplot2::scale_color_manual(values = c("Up-regulated" = "firebrick3", 
...
**Figure 2.** Volcano plot of differentially expressed genes comparing **(A)** grey matter and white matter tissues in MS patients, and **(B)** grey matter and white matter tissues in CON patients.
...
For the MS patients, the most up-regulated gene was `SNAP25` with a logFC of `6.482862` and the most down-regulated gene was `CNTN2` with a logFC of `-6.641904`.
For the CON patients, the most up-regulated gene was `FPR2` with a logFC of `3.2941815` and the most down-regulated gene was `CFAP43` with a logFC of `-7.137068`.

## Heat map

We can better visualize global expression change over our samples with a heat map.
In this heatmap, all the groups - CON GM, CON WM, MS GM, MS WM will be compared against each other.
...
**Figure 3.** Heat map of global gene expression across all samples.
Samples were annotated based on their patient group and tissue on top of each column, its legend is found on the right.
...
I first used the conventionally used p-value of 0.05 for both sets of analysis.
This resulted to `572` genes that were differentially expressed in `GM` between MS and CON, and `729` genes that were differentially expressed in `WM` between MS and CON.
The comparison of the two tissues, GM and WM, resulted to `3042` genes that were differentially expressed in `MS` patients, and `2901` genes that were differentially expressed in `CON` patients.
Since there were still many genes, I reduced the p-value to 0.01.
This decreased the number of DE genes to `113`, `176`, `1621`, and `1498` respectively.

### 2. Multiple hypothesis testing - correct your p-values using a multiple hypothesis correction method. Which method did you use? And Why? How many genes passed correction?

We used the Benjamini-Hochberg correction.
Benjamini-Hochberg differs from Bonferroni in that it limits the False Discovery Rate (FDR).
We assume that the power to detect differential expression is equally likely among all the tests - reducing the false discovery rate.
Additionally, Bonferroni can become conservative as the number of comparisons increases so this method is not ideal for the number of samples we have [@chen2017].
This correction was already done before we found the number of DE genes so the numbers mentioned in Q1 covers the number of genes that passed correction after changing the p-value.

### 3. Show the amount of differentially expressed genes using an MA Plot or a Volcano plot. Highlight genes of interest.

Volcano plot for MS and CON patients with the comparison of GM vs WM tissue samples.
The top DE genes are labelled for each plot.

```{r message = FALSE, tidy=TRUE, warning=FALSE}
knitr::include_graphics("~/projects/A2/figures/a2_volcanoms.png")
```

```{r message = FALSE, tidy=TRUE, warning=FALSE}
knitr::include_graphics("~/projects/A2/figures/a2_volcanocon.png")
```
...
```{r message = FALSE, tidy=TRUE, warning=FALSE}
knitr::include_graphics("~/projects/A2/figures/a2_heatmap.png")
```

For both patient groups, we can see similarities in the genes that down- and up-regulated, i.e. genes that are up-regulated in GM in CON patients will be up-regulated in MS patients.
This relates to what we saw in our MDS plot from A1 that was shown in the introduction section of this assignment.
Samples of the same tissue cluster regardless of patient group.
...
To perform the gene enrichment analysis, we will use the R package for g:Profiler.
We used FDR for correction.
In our previous course journal entry assignment, we used GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP) - we will be using these sources in this section.
...
<br>
**Table 2.** The first top 10 terms across all the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 3.** The top term from each of the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 4.** The first top 10 terms across all the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 5.** The top term from each of the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 6.** The first top 10 terms across all the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 7.** The top term from each of the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 8.** The first top 10 terms across all the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 9.** The top term from each of the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 10.** The first top 10 terms across all the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 11.** The top term from each of the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.

As we saw earlier from the up-regulated analysis for the CON samples, the WP source had 0 terms.
We have a top term from WP in the down-regaulted genes as seen in Table 9.
...
<br>
**Table 13.** The first top 10 terms across all the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.
...
<br>
**Table 14.** The top term from each of the three sources, GO Biological Process (<GO:BP>), Reactome (REAC), and WikiPathways (WP).
The ID of each term is also included.

## Thresholded over-representation analysis discussion

### 1. Which method did you choose and why?
I used g:Profiler in this assignment as we have gone over this method in lecture and in the previous journal entry assignment. Additionally, they update the annotation data regularly across many evidence types, identifier spaces and organisms. The parameters, significance thresholds, term sizes, and correction method, are also customizable. 


### 2. What annotation data did you use and why? What version of the annotation are you using?
I chose GO:BP (releases/2022-12-04), Reactome (releases/2022-12-28), and WikiPathways (releases/2022-12-10) as we have already used them for the previous journal entry assignment. GO:BP and Reactome support the most up-to-date annotation of human genes, which is highly wanted in our case since we are looking at a human disease. WikiPathways covers many types of molecular pathways.


### 3. How many genesets were returned with what thresholds?
I used a threshold of adjusted p-value < 0.01, and term size between 1 and 500.

MS GM vs. WM: <br>
Up-regulated: `r length(msUpTopTerms$term_name)` gene sets \
Down-regulated: `r length(msDownTopTerms$term_name)` genes sets \
All DE genes returned `r length(msAllTopTerms$term_name)` gene sets.

CON GM vs. WM: <br>
Up-regulated: `r length(conUpTopTerms$term_name)` gene sets \
Down-regulated: `r length(conDownTopTerms$term_name)` gene sets \
All DE genes returned `r length(conAllTopTerms$term_name)` gene sets.


### 4. Run the analysis using the up-regulated set of genes, and the down-regulated set of genes separately. How do these results compare to using the whole list (i.e all differentially expressed genes together vs. the up-regulated and down regulated differentially expressed genes separately)?
Overall, using the whole list of genes returned much more gene sets than running up-regulated and down-regulated genes separately (as seen in the previous question). There were also much more gene sets for the down-regulated genes than up-regulated genes in both comparisons. This may suggest that there are more genes that are down-regulated that are of biological significance. Additionally, for the analysis with all DE genes, the top terms were very general, e.g. RNA splicing, mRNA processing, and histone modification. Running the genes separately yielded more terms that were relevant to the research topic like immunopathways such as IL-18 signaling pathway, NF-kB pathway, and myeloid leukocyte mediated immunity.


# Task 3: Interpretation

### 1. Do the over-representation results support conclusions or mechanism discussed in the original paper?
Yes, our over-representation analysis results support the conclusions or mechanisms discussed in the paper by van der Poel et al. [@vanderPoel2019]. The paper attempts to define a region-specific transcriptional profile for human microglia in GM and WM tissue, and our analysis has shown that there are many DE genes between the two tissues in both patient groups. Additionally, macrophage- and myeloid-related pathways have also been shown in our top terms as was seen in the paper. 


### 2. Can you find evidence, i.e. publications, to support some of the results that you see. How does this evidence support your results.
It has already been shown that the NF-kB pathway is essential in the pathology of MS and is one of the terms that has shown up in our analysis [@mcguire2013]. Our analysis has shown associations with pathways related to myeloid mediated immunity, and myeloid cell activation. MS is a demyelinating disease and the demyelination of the myelin sheath leads to the formation of lesions/plaques. There were many terms related to lipid metabolism which relates back to MS pathology - there is a complex arrangement of lipids within the myelin sheath [@podbielska2021] and gene expression within these pathways can point to evidence of lipid markers used in the prediction of the course of MS. Lesions are classically found in the white matter of the brain. Lesions in MS are classically found in the WM regions of the brain and, interestingly, WM regions are lipid-rich [@vanderPoel2019].

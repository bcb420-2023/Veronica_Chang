...
dgeCounts <- edgeR::calcNormFactors(counts_dge)
dgeCountsAll <- edgeR::cpm(counts_dge)
...
```
**Figure 2.** Volcano plot of differentially expressed genes comparing (A) MS and CON patients in grey matter tissue samples, and (B) MS and CON patients in white matter tissue samples. Differentially expressed genes are coloured based on whether they are up- or down-regulated. Genes were selected when FDR < 0.05 AND logFC > 0 for up-regulated OR logFC < 0 for down-regulated. If they do not fulfill these requirements, they are labelled as not differentially expressed (DE).

There are only a few DE genes (4 genes) and all of them are up-regulated in MS patients for each tissue.
The gene with the greatest logFC is `ELK2AP` with a logFC of `10.7060661`.
...
For the MS patients, the most up-regulated gene was `SNAP25` with a logFC of `6.482862` and the most down-regulated gene was `GABRG1` with a logFC of `-7.035827`. For the CON patients, the most up-regulated gene was `MMP25` with a logFC of `5.341661` and the most down-regulated gene was `SPAG17` with a logFC of `-7.424698`.
...
hmapRange <- range(heatmapMatrix)
hmapCol <- circlize::colorRamp2(c(hmapRange[1], 0, hmapRange[2]), 
                                 c("blue", "white", "red"))

# Create heat map
hmapDEG <- ComplexHeatmap::Heatmap(heatmapMatrix,
                                    show_row_dend = FALSE,
...
We now want to also adjust the p-values using the Benjamini-Hochberg method as we are doing multiple comparisons.

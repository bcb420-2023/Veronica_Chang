...
## Dataset
After our previous analysis, the final dataset was made up of:
...
## Previous Analysis
In the previous assignment, A1, we used the dataset ID [GSE111972](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE111972), from the publication "Transcriptional profiling of human microglia reveals grey-white matter heterogeneity and multiple sclerosis-associated changes" by van der Poel et. al (@vanderPoel2019). In A1, we conducted data cleaning, normalization, gene mapping, and preliminary analysis. We filtered out the lowly expressed genes and were left with 15076 genes compared to the original 21283. A total of 6207 genes were removed. The number of genes did not change after normalization. Genes were already mapped to HUGO gene symbols. When we generated a MDS plot post-normalization, we saw that the tissues GM and WM clustered relatively well with each other regardless of which patient group they were obtained from.
...
I first used the conventionally used p-value of 0.05 for both sets of analysis. This resulted to `572` genes that were differentially expressed in `GM` between MS and CON, and `729` genes that were differentially expressed in `WM` between MS and CON. The comparison of the two tissues, GM and WM, resulted to `3042` genes that were differentially expressed in `MS` patients, and `2901` genes that were differentially expressed in `CON` patients. Since there were still many genes, I reduced the p-value to 0.01. This decreased the number of DE genes to `113`, `176`, `1621`, and `1498` respectively. 

<br>
### 2. Multiple hypothesis testing - correct your p-values using a multiple hypothesis correction method. Which method did you use? And Why? How many genes passed correction?
We used the Benjamini-Hochberg correction. Benjamini-Hochberg differs from Bonferroni in that it limits the False Discovery Rate (FDR). We assume that the power to detect differential expression is equally likely among all the tests - reducing the false discovery rate. Additionally, Bonferroni can become conservative as the number of comparisons increases so this method is not ideal for the number of samples we have. This correction was already done before we found the number of DE genes so the numbers mentioned in Q1 covers the number of genes that passed correction after changing the p-value.

<br>
### 3. Show the amount of differentially expressed genes using an MA Plot or a Volcano plot. Highlight genes of interest.
Volcano plot for MS and CON patients with the comparison of GM vs WM tissue samples. The top DE genes are labelled for each plot. 
```{r message = FALSE, tidy=TRUE, warning=FALSE}
...
```
For both patient groups, we can see similarities in the genes that down- and up-regulated, i.e. genes that are up-regulated in GM in CON patients will be up-regulated in MS patients. This relates to what we saw in our MDS plot from A1 that was shown in the introduction section of this assignment. Samples of the same tissue cluster regardless of patient group. 

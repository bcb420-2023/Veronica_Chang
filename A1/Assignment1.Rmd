...
summaryCONWM <- summary(rowMeans(expDataFiltered[, grep("Sample_CON_WM", colnames(expDataFiltered))]))
# MS (multiple sclerosis) WM (white matter)
summaryMSWM <- summary(rowMeans(expDataFiltered[, grep("Sample_MS_WM", colnames(expDataFiltered))]))
# CON (non-neurological control) GM (grey matter)
summaryCONGM <- summary(rowMeans(expDataFiltered[, grep("Sample_CON_GM", colnames(expDataFiltered))]))
# MS (multiple sclerosis) GM (grey matter)
summaryMSGM <- summary(rowMeans(expDataFiltered[, grep("Sample_MS_GM", colnames(expDataFiltered))]))
...
edgeR::plotMDS.DGEList(d, labels=rownames(samples),
  col = c("darkgreen","blue")[factor(samples$patients_group)])
```
